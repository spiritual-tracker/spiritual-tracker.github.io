// React Native Date Utilities
// Mirrors the web app date functionality for mobile

// Get user's timezone (simplified for mobile)
...
// Set user timezone (stored in AsyncStorage)
export const setUserTimezone = async (timezone) => {
  try {
    const AsyncStorage = require('@react-native-async-storage/async-storage').default;
    await AsyncStorage.setItem('spiritual_tracker_timezone', timezone);
  } catch (error) {
    console.error('Error saving timezone:', error);
  }
};

// Get available timezones (simplified list for mobile)
